...
object EtlConfiguration extends App {
  ...
  val sources =
    List(
      ...
      DatasetConf("enriched_snv_somatic"           , clin_datalake, "/enriched/snv_somatic"              , DELTA  , OverWritePartition, partitionby = List("analysis_service_request_id"), table = Some(TableConf("clin", "snv_somatic"))      , keys = List("chromosome", "start", "reference", "alternate", "aliquot_id", "bioinfo_analysis_code")),
